...
class OffsetTracker {
        ...
        offsetsByPartition.forEach(((topicPartition, offsets) -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (offsetState.isProcessed()) {
                    ...
                    System.out.println("-----offset="+offsetState.offset);
                    if (max == null || offsetState.offset > max-1) {
                        max = offsetState.offset+1;
                        newMaxFound = true;
                    }
                } else {
                    break;
    ...
    public Map<TopicPartition, Long> getMaxOffsetByPartition() {
        return maxOffsetByPartition;
    }

    public static void main(String[] args) {
        OffsetTracker offsetTracker = new OffsetTracker();
        SinkRecord sinkRecord = new SinkRecord("myTopic",0,null,null,null,"test",123);
        OffsetState offsetState = offsetTracker.addPendingRecord(sinkRecord);

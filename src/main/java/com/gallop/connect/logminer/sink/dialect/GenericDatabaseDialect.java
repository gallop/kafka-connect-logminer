...
package com.gallop.connect.logminer.sink.dialect;

import com.gallop.connect.logminer.sink.*;
import com.gallop.connect.logminer.sink.metadata.FieldsMetadata;
import com.gallop.connect.logminer.sink.metadata.SchemaPair;
import com.gallop.connect.logminer.sink.metadata.SinkRecordField;
import com.gallop.connect.logminer.source.LogMinerSourceConnectorConfig;
import com.gallop.connect.logminer.util.*;
import com.gallop.connect.logminer.util.ColumnDefinition.Nullability;
import com.gallop.connect.logminer.util.ColumnDefinition.Mutability;
import com.gallop.connect.logminer.util.ExpressionBuilder.Transform;
import com.gallop.connect.logminer.sink.JdbcSourceConnectorConfig.NumericMapping;
import com.gallop.connect.logminer.sink.JdbcSinkConfig.PrimaryKeyMode;
import com.gallop.connect.logminer.sink.JdbcSinkConfig.InsertMode;
...
public class GenericDatabaseDialect implements DatabaseDialect {
  ...
  protected GenericDatabaseDialect(
    ...
    this.jdbcUrl = config.getString(LogMinerSourceConnectorConfig.CONNECTION_URL_CONFIG);
  ...
  @Override
  public Statement createStatement(Connection db) throws SQLException {
  ...
  public TableId parseTableIdentifier(String fqn) {
    ...
    //parts.forEach(e-> System.out.println("------>>>>>parts:"+e));
    if (parts.isEmpty()) {
